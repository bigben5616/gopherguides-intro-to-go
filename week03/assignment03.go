...
func (a Movie) String() string{
  return fmt.Sprintf("%s(%da) %.1f%%", a.Name, a.Length, a.Rating()) //The decimal place on the float is just one byte by using %.1f%%. We will be interpreting a percent sign and the second % sign is telling Go there is a %
